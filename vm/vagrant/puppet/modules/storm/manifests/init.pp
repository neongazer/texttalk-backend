class storm {
	...
	exec { "storm-dowload":
		command => "wget -N http://www.mirrorservice.org/sites/ftp.apache.org/incubator/storm/$storm_version/$storm_binary",
		path => "/usr/bin/:/bin/",
		cwd => "/tmp/",
  ...
  file { "storm-log-config-path":
    path => "/usr/share/storm/log4j",
    ensure => directory,
    owner => "storm",
    group => "storm",
    require => [Group["storm"],User["storm"]],
